...
@app.route("/save_and_analyze", methods=["POST"])
def handle_save_and_analyze():
    global global_state
    data = request.json
    ...
    if content:
        try:
            file_path = os.path.join(SERVICE_AGREEMENT_UPLOAD_FOLDER, "extracted_rules_new.txt")
            with open(file_path, "w", encoding="utf-8") as file:
                file.write(content)
            update_status("Agreement content saved successfully.")
            global_state["save_and_analyze"] = True
            return jsonify({"message": "Save and Analyze triggered.", "success": True})
        except Exception as e:
            return jsonify({"message": f"Error saving agreement content: {e}", "success": False})
    else:
        return jsonify({"message": "No agreement content provided.", "success": False})
...
def upload_files():
    ...
    return render_template("upload_2.html", file_exists=file_exists, existing_rules=existing_rules)
...
def run_analysis(state):
    ...
    def await_user_decision(state):
        global global_state
        service_agreement_option = state.get("service_agreement_option")

        if service_agreement_option == "new":
            # Wait for contract rules to be generated
            update_status("Do you want to view/edit the generated rules?")
        else:
            # Immediately ask if they want to view/edit the existing rules
            update_status("Do you want to view/edit the existing rules?")
        ...
        if decision == "yes":
            update_status("Loading the rules to editable area.")
            extracted_rules_path = os.path.join(SERVICE_AGREEMENT_UPLOAD_FOLDER, "extracted_rules_new.txt" if service_agreement_option == "new" else state["service_agreement"])
            ...
            # Wait for user to click "Save and Analyze"
            while not global_state.get("save_and_analyze"):
                time.sleep(1)
            return state
        elif decision == "no":
            update_status("User decided not to view/edit the rules. Proceeding with data extraction.")
            #state = data_extraction_task(state)  # Trigger data extraction
            return state
        
        return state
        
    def save_and_analyze(state):
        update_status("Saving edited rules and analyzing...")
        
        # Retrieve the edited rules from the state
        edited_rules = state.get("extracted_rules")
        file_path = os.path.join(SERVICE_AGREEMENT_UPLOAD_FOLDER, "extracted_rules_new.txt")
        
        # Save the edited rules to the file
        if edited_rules:
            try:
                with open(file_path, "w", encoding="utf-8") as file:
                    file.write(edited_rules)
                update_status("Edited rules saved successfully.")
            except Exception as e:
                update_status(f"Error saving edited rules: {e}")
                return state
        else:
            update_status("No edited rules found to save.")
        
        # Proceed with data extraction
        #state = data_extraction_task(state)
        
        return data_extraction_task(state)
    ...
    builder.add_node("save_and_analyze", save_and_analyze)
    ...
    builder.add_conditional_edges(
        ...
        ["rules_extractor", "await_user_decision"],
    )
    ...
    builder.add_conditional_edges(
        "await_user_decision",
        route_user_decision,
        ["save_and_analyze","data_extraction"]
    )
    
    #builder.add_edge("await_user_decision", "data_extraction")
    builder.add_edge("save_and_analyze", "data_extraction")
